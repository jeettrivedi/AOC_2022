...
def sol(input_file_path: str):
    ...
    for direction, step in steps:
        for _ in range(int(step)):
            # Move head
            ROPE[0]["position"] = add_vectors(
                ROPE[0]["position"], BASIS_VECTORS[direction]
            )

            # Update rope position
            for idx in range(1, ROPE_LENGTH):
                displacement = vector_displacement(
                    ROPE[idx]["position"], ROPE[idx - 1]["position"]
                )
                moves = movement_vector(displacement)
                for tail_move in moves:
                    ROPE[idx]["position"] = add_vectors(
                        ROPE[idx]["position"], BASIS_VECTORS[tail_move]
                    )
